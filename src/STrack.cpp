...
byte_track::STrack::STrack(const Rect<float>& rect, const float& score, const std::string& class_name, const size_t& label_id)
    : kalman_filter_()
    , mean_()
    , covariance_()
    , rect_(rect)
...
const size_t& byte_track::STrack::getLabelId() const {
    return label_id_;
}
...
void byte_track::STrack::update(const STrack& new_track, const size_t& frame_id) {
    ...
    score_ = new_track.getScore();
    frame_id_ = frame_id;
    class_name_ = new_track.class_name_;
    label_id_ = new_track.label_id_;
    tracklet_len_++;
}
